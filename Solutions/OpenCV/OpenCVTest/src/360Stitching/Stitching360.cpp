...
private:
	...
	cv::Mat									m_cmMap1;					/* 最终矫正的映射表 */
	cv::Mat									m_cmMap2;					/* 最终矫正的映射表 */
...
public:
	...
	virtual cv::Mat Undistort(cv::Mat &mSrcImg);
...
Stitching360::Stitching360() :m_sImageRoot("input\\"), m_sLastName(".png"), m_sCaliResult("out\\result.txt"), m_szBoard(cv::Size(7, 7)), m_nImageCount(15), m_nSuccessImageNum(0)
{
}
...
int Stitching360::Init(int nSrcHeight, int nSrcWidth)
{
	...
	cv::fisheye::initUndistortRectifyMap(m_mIntrinsicMatrix, m_vDistortionCoeffs, cv::Matx33d::eye(), m_mNewIntrinsicMat, m_szImage, CV_32FC1, map1, map2);
...
int Stitching360::findCorners()
{
	...
	for (int i = 0; i != m_nImageCount; i++)
	{
		...
		bool patternfound = cv::findChessboardCorners(image, m_szBoard, n_vCorners, cv::CALIB_CB_ADAPTIVE_THRESH + cv::CALIB_CB_NORMALIZE_IMAGE + cv::CALIB_CB_FAST_CHECK);
...
int Stitching360::cameraCalibrate(int count) {
	...
	cv::fisheye::estimateNewCameraMatrixForUndistortRectify(m_mIntrinsicMatrix, m_vDistortionCoeffs, image_size, cv::noArray(), m_mNewIntrinsicMat, 0.5, image_size, 1.0);
	std::cout << "定标完成！\n";
	return 1;
}
...
cv::cuda::GpuMat Stitching360::Undistort(cv::cuda::GpuMat &mSrcImg)
{
	...
	//cv::remap(mSrcImg, mDstImg, map1, map2, cv::INTER_LINEAR, cv::BORDER_CONSTANT);
	//cv::cuda::remap(mSrcImg, mDstImg, m_cmMap1, m_cmMap2, cv::INTER_LINEAR, cv::BORDER_CONSTANT);
	
	return mDstImg;
}

cv::Mat Stitching360::Undistort(cv::Mat &mSrcImg)
{
	cv::Mat mDstImg;
